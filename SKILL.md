...
| 03 心率强度与有氧效率 | Z1–Z5 时长占比 + **逐课 100% 区间堆叠柱** + **配速-心率散点拟合**（含漂移/目标配速预测心率） |
...
`c11` 逐课心率区间 100% 堆叠柱 / `c12` 配速-心率效率散点（含线性拟合线）

### c11 / c12 的数据来源（新增图表）

- `c11` 用 `activities[].hr_zones[].seconds`，按活动归一化为百分比；时长 < 60s 的活动跳过。
  一眼看出哪堂课强度失控（橙红段占大头）。
- `c12` 用 `activities[].splits_km[].pace_s` 与 `avg_hr`，一公里一个点，按活动分色；
  过滤 `150 ≤ pace_s ≤ 1500`、`90 ≤ hr ≤ 220`；分组少于 3 点则不入图。
  x 轴 `reverse: true`（向右更快），红色虚线为最小二乘拟合。
- 配套结论卡：拟合式 + R²、**目标配速代入后的预测心率 vs 乳酸阈占比**、
  最长一次的**有氧漂移**（前后各 1/3 公里效率比 EF=速度/心率 的衰减，≥5% 提示耐力不足）。

> ⚠️ `build_efficiency()` 的 `target_pace` 来自 `config.goals.target_pace_sec_per_km`；
> 没配就只出散点、不出预测心率那行，不会报错。
